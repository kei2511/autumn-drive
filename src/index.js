...
app.get("/", (req, res) => {
  res.json({
    status: "online",
    message: "Autumn Drive Backend is Running 🍂☁️",
    version: "1.0.0",
  });
});
...
logger.log("[Startup] Initializing Autumn Drive (Supabase Mode)...");
...
if (process.env.NODE_ENV !== "test") {
  storage.init().then(() => {
    // 3. Start Server
    app.listen(PORT, () => {
      logger.log(`Autumn Drive backend running on http://localhost:${PORT}`);
    });
  });
}
