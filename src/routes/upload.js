...
router.post("/chunk", authMiddleware, (req, res) => {
...
router.post("/finalize", [authMiddleware, express.json()], async (req, res) => {
  ...
  try {
    const metadata = {
      ...
      userId: req.userId, // Capture authenticated User ID
      chunks: chunks,
    };
...
router.delete("/cancel", [authMiddleware, express.json()], async (req, res) => {
